...
export default function Stages() {
  ...
  useEffect(() => {
    const stageIndexParam: string | null = searchParams.get("stageIndex");

    if (stageIndexParam !== null) {
      const index = parseInt(stageIndexParam, 10);
      if (!isNaN(index) && index >= 0 && index < caseStudies.length) {
        setCurrentPart(index);
        setStageTitle(stageParam ? decodeURIComponent(stageParam) : "Unknown Stage");
      }
    }
  }, [searchParams]);
